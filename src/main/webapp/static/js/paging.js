...
(function(root, factory) {
	...
	Paging.prototype = {
		...
		changePagesize: function(ps) {
			...
			this.settings.changePagesize && this.settings.changePagesize(ps); // 改变每页显示数据
		},
		format: function() {
			var html = '<ul>'
			// html += '<li class="js-page-first js-page-action ui-pager" >' + this.settings.firstTpl + '</li>'; // 首页

			if(this.settings.toolbar){
				html += "共&nbsp;<span class='js-page-total'>" + this.settings.count + "</span>&nbsp;条&nbsp;&nbsp;";
			}
			...
			// html += '<li class="js-page-last js-page-action ui-pager">' + this.settings.lastTpl + '</li>'; // 末页
			html += '</ul>';
		...
		bindToolbar: function() {
			var _this = this;
			var html = $('<li class="ui-paging-toolbar"><select class="ui-select-pagesize"></select>到第<input type="text" class="ui-paging-count"/>页<a href="javascript:void(0)">确认</a></li>');
