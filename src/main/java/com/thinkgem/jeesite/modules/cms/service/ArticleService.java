...
//		dc.createAlias("category.jeesite", "category.jeesite");
	...
	public void save(Article article) {
		...
		if (article.getCategory()!=null&&StringUtils.isNotBlank(article.getCategory().getId())){
			...
			if (!Global.YES.equals(category.getIsAudit())){
				//设置删除
				if (article.getDelFlag().equals(Article.DEL_FLAG_DELETE)){
					article.setDelFlag(Article.DEL_FLAG_DELETE);
				}
				//设置发布
				else if (article.getDelFlag().equals(Article.DEL_FLAG_NORMAL)){
					article.setDelFlag(Article.DEL_FLAG_NORMAL);
				}
				//设置审核
				else if (article.getDelFlag().equals(Article.DEL_FLAG_AUDIT)){
					article.setDelFlag(Article.DEL_FLAG_AUDIT);
				}
			}
		}
